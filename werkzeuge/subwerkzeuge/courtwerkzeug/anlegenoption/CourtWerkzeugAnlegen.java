 ...
 *
 */
public class CourtWerkzeugAnlegen extends AbstractSubwerkzeug {

	CourtWerkzeugAnlegenUI _ui;
		...
		_ui.get_speichernButton().setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				if (_ui.get_nameTextfield().getText().equals("")) {
					...
					Court court = new Court(_ui.get_nameTextfield().getText(),
							_ui.get_beschreibungTextfield().getText(), 1);

					ServiceManager.courtservice().fuegeCourtEin(court);
	...
	protected void initialisiereLayout() {
		_ui = (CourtWerkzeugAnlegenUI) this.getFxml("fxml/CourtWerkzeugAnlegenFxml.fxml");
	...
	private void loescheFeldeingaben() {
		_ui.get_nameTextfield().setText(null);
		_ui.get_beschreibungTextfield().setText(null);
	}
